...
public:
	...
	// Acces direct a la representation en memoire des donnees
	size_t size(void) { return m_memBuffer.size(); };
	const char* get(void) const { return m_memBuffer.data(); }
	char* get(void) { return m_memBuffer.data(); }
	
	virtual void accept(class AbsFileVisitor& v);
protected:
